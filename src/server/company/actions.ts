...
export async function saveCompanyProfileAction(_: CompanyProfileState, form: FormData): Promise<CompanyProfileState> {
  ...
  if (!parsed.success) return { error: copy.validationError, fieldErrors: fieldErrors(parsed.error.issues) };
  ...
  if (!industry) return { error: copy.invalidIndustry, fieldErrors: { industry_key: copy.invalidIndustry } };
  if (!revenue) return { error: copy.invalidRevenue, fieldErrors: { annual_revenue_key: copy.invalidRevenue } };
  ...
  if (readError) return { error: copy.saveError };
  const expectedRevision = Number(value(form, "revision") || "0");
  if (existing && expectedRevision > 0 && expectedRevision !== existing.revision) return { error: copy.conflictError };
  ...
  if (result.error || !result.data) return { error: result.error?.code === "23505" ? copy.duplicateError : copy.saveError };
