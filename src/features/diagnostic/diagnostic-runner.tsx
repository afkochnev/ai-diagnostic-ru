...
export function DiagnosticRunner({ data }: Props) {
  ...
  const save = useCallback(async (nextBlockId = currentBlock.id) => {
    ...
    const operation = saveQueue.current.then(async () => {
      ...
      try {
        ...
        revisionRef.current = body.revision; persistedAnswersHashRef.current = currentHash; setRevision(body.revision); setSaving("saved");
        savedHideTimer.current = setTimeout(() => { setSaving("idle"); savedHideTimer.current = null; }, 1500);
        return true;
      } catch {
  ...
  useEffect(() => () => { if (savedHideTimer.current) clearTimeout(savedHideTimer.current); }, []);
